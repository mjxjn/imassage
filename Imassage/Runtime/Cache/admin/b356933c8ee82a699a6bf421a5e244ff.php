...
<div class="centercontent">
	<div class="pageheader">
            ...
            <span class="pagedesc">此页面用于添加查看某位按摩师的服务项目</span>
    </div><!--pageheader-->
    <div id="contentwrapper" class="contentwrapper">
    	<div id="basicform" class="subcontent">
                    ...
                    <form id="pform" class="stdform" action="" method="post">
                    	<p>
                        	<label>按摩师名</label>
                            <span class="field"><input type="text" name="name" id="name" value="<?php echo ($name); ?>" disabled="disabled" class="mediuminput" /></span>
                        </p>
                        <p>
                            <label>服务项目</label>
                            <span id="dualselect" class="dualselect">
                                <select class="uniformselect" name="plist" multiple="multiple" size="10">
                                ...
                                <select name="products[]" id="plist2" multiple="multiple" size="10">
                                    <?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["pid"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                            </span>
                        </p>
                        <p class="stdformbutton">
                            <button class="submit radius2">提交修改</button><input type="hidden" name="id" value="<?php echo ($id); ?>" />
...
jQuery(document).on('click', '.submit', function() {
        //var id = ""; //定义数组
        jQuery("#plist2").each(function(){ //遍历全部option
            jQuery("#plist2 option").attr("selected",true); //获取option的内容
